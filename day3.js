...
const checkNumberChar = (char) => char ? /[0-9]/.test(char) : false;
...
for(let i = 0; i < engine.length * engine[0].length; i++) {
  const y = parseInt((i) / engine[0].length);
  const x = parseInt((i) % engine[0].length);
...
console.log(array)
for(item of array) {
  for(let addToIndex = 0; addToIndex < item.length; addToIndex++) {
    const y = parseInt((item.index + addToIndex) / engine[0].length);
    const x = parseInt((item.index + addToIndex) % engine[0].length);
    let needBreak = false;
    if(checkAround(y, x)) {
      answer += item.number;
      needBreak = true;
