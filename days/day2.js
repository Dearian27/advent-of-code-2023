const lineX = `Game 1: 1 green, 6 red, 4 blue; 2 blue, 6 green, 7 red; 3 red, 4 blue, 6 green; 3 green; 3 blue, 2 green, 1 red
...
const games = lineX.split('\n');

games.forEach((game, index) => {
  const lineWithoutGameN = game.replace(regexp, '');
  const gamesArray = lineWithoutGameN.split('; ');
  ...
  gamesArray.forEach(gameSet => {
    let cubes = gameSet.split(', ');
    const rgb = {
      red: 0,
      green: 0,
      blue: 0,
    ...
    cubes.forEach(c => {
      Object.keys(rgbConsumes).map(rgbConsume => {
        if(c.match(rgbConsume)) {
          const count = c.match(/^(\d+)*/)[0];
          if(minRGB[rgbConsume] === 0 || minRGB[rgbConsume] <= parseInt(count))  {
            // console.log('min value of ' +  rgbConsume + ': ' + count)
            minRGB[rgbConsume] = parseInt(count);
          }
          rgb[rgbConsume] += parseInt(count);
        }
      })
    })
    Object.keys(rgbConsumes).map((col) => {
      if(rgbConsumes[col] < rgb[col]) {
        // console.log('impossible: ' + col + rgb[col]);
        impossible = true;
      }
    })
  })
  ...
  possibleGames += minRGB.blue * minRGB.red * minRGB.green;
  // }
})
